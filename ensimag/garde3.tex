...
\newcommand{\smu}{\smallcaps{Smart Me Up}}
...
\section Le rapport de stage AI est rédigé en anglais.
...
\section{Goal of the internship}
...
used \ffmpg/\avconv, but we finnally found that \gstreamer as better performance
results.
...
c++ -> native code
\gstreamer pipeline
reduce dependency -> remove boost
...
All what I done is still more a proof of concept than something production
ready.

can compile and run on multiple plateforms~: all \linux (both intel and arm) and
\windows. A lots of work have also be done for android, but all the work have
not be finished.

need a lots more tuning
